...
function NavPillLink({
  ...
  const className = cn(
    'group flex flex-col items-center gap-1 text-sm font-semibold text-slate-700 transition-colors hover:text-brand',
    'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40 focus-visible:ring-offset-2 rounded-lg px-1 py-0.5'
  )
  ...
  if (external || href.startsWith('mailto:')) {
    return (
      <a href={href} className={className}>
        <ChevronDown
          className="h-3.5 w-3.5 shrink-0 text-slate-400 transition-colors group-hover:text-brand"
          aria-hidden
        />
        <span>{children}</span>
      </a>
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-0 z-50 flex justify-center px-4 pt-4 sm:px-6 sm:pt-5">
      ...
      >
        ...
        {/* المنتصف: الروابط + اللغة (تمرير أفقي على الشاشات الضيقة) */}
        <div
          className={cn(
            'flex min-w-0 flex-1 items-center justify-center gap-3 sm:gap-5 md:gap-6 lg:gap-8',
            'overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden'
          )}
        >
          <div className="flex shrink-0 items-center gap-4 sm:gap-6 lg:gap-8">
            <NavPillLink href="/#features">{t('أقسام بلانورا', 'Planora sections')}</NavPillLink>
            <NavPillLink href="/blog">{t('المدونة', 'Blog')}</NavPillLink>
            <NavPillLink href="mailto:customerservice@planora.app" external>
          ...
          <button
            ...
            className={cn(
              'inline-flex shrink-0 items-center gap-1.5 rounded-full border border-slate-200 bg-slate-50/80 px-3 py-2 text-xs font-semibold text-slate-700 transition-colors hover:border-brand/30 hover:bg-white hover:text-brand sm:text-sm'
            )}
        ...
        {/* RTL: آخر عنصر = اليسار — تسجيل الدخول */}
        <Link
          href="/login"
          className={cn(
            'shrink-0 rounded-full bg-brand px-4 py-2.5 text-sm font-bold text-white shadow-md shadow-brand/25 transition-colors hover:bg-brand-dark',
            'sm:px-5 sm:py-2.5'
          )}
        >
